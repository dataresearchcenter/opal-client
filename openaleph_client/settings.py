...
def get_env_var(env_var_name, default_value):
    for complete_env_var_name in [f"MEMORIOUS_{env_var_name}", f"OPENALEPH_{env_var_name}", f"OPAL_{env_var_name}"]:
...
DATABASE_URI = get_env_var("DATABASE_URI", "postgresql://opal:opal@localhost/opal")
...
FILE_BATCH_SIZE = int(os.environ.get("OPENALEPH_FILE_BATCH_SIZE", 1000))